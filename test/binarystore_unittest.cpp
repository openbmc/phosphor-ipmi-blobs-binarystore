...
TEST_F(BinaryStoreTest, TestWriteExceedMaxSize)
{
    ...
    EXPECT_TRUE(
        store->write(0, writeData)); // 44 =  10 (data) + 2 (proto header)
    EXPECT_FALSE(store->write(10, writeData)); // 54 = 10 (new data)
    EXPECT_FALSE(store->write(5, writeData));  // 49 = 5 (new data)
}
